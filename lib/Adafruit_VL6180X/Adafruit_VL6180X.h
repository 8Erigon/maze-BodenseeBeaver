...
/*
Changed by Erik Bergmann:
added virtual to functions of class
*/
...
public:
  ...
  ~Adafruit_VL6180X();
  virtual boolean begin(TwoWire *theWire = &Wire);
  virtual boolean setAddress(uint8_t newAddr);
  virtual uint8_t getAddress(void);

  virtual uint8_t readRange(void);
  virtual float readLux(uint8_t gain);
  virtual uint8_t readRangeStatus(void);

  virtual boolean startRange(void);
  virtual boolean isRangeComplete(void);
  virtual boolean waitRangeComplete(void);
  virtual uint8_t readRangeResult(void);

  virtual void startRangeContinuous(uint16_t period_ms = 50);
  virtual void stopRangeContinuous(void);
  // readRangeResult and isRangeComplete apply here is well

  virtual void setOffset(uint8_t offset);
  virtual void getID(uint8_t *id_ptr);
